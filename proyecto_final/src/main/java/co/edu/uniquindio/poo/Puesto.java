...
public class Puesto {
    ...
    */
    ...
    /* 
    // Método para remover vehiculos
    */
    public void removerVehiculo(String placa) {
        for (int i = 0; i < listaPuestos.length; i++) {
            for (int j = 0; j < listaPuestos[i].length; j++) {
                if (listaPuestos[i][j] != null && listaPuestos[i][j].getPlaca().equals(placa)) {
                    
                    listaPuestos[i][j] = null;
                    return; 
                }
            }
        }
        /* 
        // esto por si la palca dada no coincide con ningun vehiculo en el parqueadeo
       */
